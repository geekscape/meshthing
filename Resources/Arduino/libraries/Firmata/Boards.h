...
// Meshthing
// May not be correct entirely.
#elif defined(__AVR_ATmega256RFR2__) || defined(__AVR_ATmega2564RFR2__)
#define TOTAL_ANALOG_PINS       8 // 7 + 1 dummy
#define TOTAL_PINS              29 // 21 digital + 7 analog + 1 dummy 
#define VERSION_BLINK_PIN       13
#define IS_PIN_DIGITAL(p)       ((p) >= 0 && (p) < TOTAL_PINS)
#define IS_PIN_ANALOG(p)        ((p) >= 21 && (p) != 25 && (p) < TOTAL_PINS )
#define IS_PIN_PWM(p)           digitalPinHasPWM(p)
#define IS_PIN_SERVO(p)         ((p) >= 2 && (p) - 2 < MAX_SERVOS)
#define IS_PIN_I2C(p)           ((p) == 9 || (p) == 16)
#define PIN_TO_DIGITAL(p)       (p)
#define PIN_TO_ANALOG(p)        (p - 21)
#define PIN_TO_PWM(p)           PIN_TO_DIGITAL(p)
#define PIN_TO_SERVO(p)         ((p) - 2)
